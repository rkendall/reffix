...
var config = {
	...
	set: function(optionsFromCommandLine, pathOfCustomConfigFile) {
		...
		if (optionsFromCommandLine) {
			...
			if (optionsFromCommandLine.workingDirectory) {
				settings.workingDirectory = path.resolve(process.cwd(), optionsFromCommandLine.workingDirectory);
			}
			if (optionsFromCommandLine.directoriesToExclude) {
				// TODO Avoid adding duplicates to array
				settings.directoriesToExclude = settings.directoriesToExclude.concat(optionsFromCommandLine.directoriesToExclude);
			}
	...
	// TODO Better name for this; or arg that specifies whether to overwrite directory exclusions
	...
	replaceVariables: function(settings) {
		for (var mode in settings) {
			...
			// Set '.' or './' to cwd
			if (/\.\/?/.test(settingsForMode.workingDirectory)) {
				settingsForMode.workingDirectory = process.cwd();
			}
		}
		return settings;
	},
