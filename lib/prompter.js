		...
		if (numberOfFilesToFix || Object.keys(referencesWithMultiplePossibleCorrections).length) {
			...
			.then(function(confirmed) {
				if (confirmed.correct) {
					return self.correctAndReport(brokenReferences, existingFiles);
				}
				return false;
			})
			.then(function(promptToFix) {
				if (promptToFix) {
					self.promptToSelectCorrectPath(brokenReferences, existingFiles);
				}
			})
			.catch(function(err) {
				console.error(err.message, err.stack);
	...
	promptToSelectCorrectPath: function(brokenReferences, existingFiles) {
		...
		inquirer.prompt(questions)
			.then(function(answers) {
				...
				self.correctAndReport(fixedReferences, existingFiles);
			}).catch(function(err) {
				console.error(err.message, err.stack);
			});
	},
