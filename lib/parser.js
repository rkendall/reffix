...
var parser = {
	...
	getReferences: function(options) {
		...
		return parser.getDataFromFiles(options)
	...
	getDataFromFiles: function(options) {
		if (options) {
			Object.assign(config, options);
		}
		var promises = [];
		...
		console.log('\nProcessing files in ' + readFileOptions.root);
		...
		if (utils.isFileIncluded(pathOfFileContainingReference, config.referencingFileFilter)) {
			...
			patterns.forEach(function(pattern) {
				...
				while (match = XRegExp.exec(fileContent, pattern, pos)) {
					...
					if (utils.isFileIncluded(absolutePathOfReference, config.referencedFileFilter)) {
