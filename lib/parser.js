...
var parser = {
	...
	getReferencesWithinFile: function(pathOfFileContainingReference, fileContent, referencedFiles) {
		...
		if (utils.isFileIncluded(pathOfFileContainingReference, config.referencingFileFilter)) {
			...
			var test = XRegExp.exec(fileContent, pattern);
			XRegExp.forEach(fileContent, pattern, function(generalMatches) {
				var statement = generalMatches[0];
				var pathPattern = XRegExp(config.replacementPattern);
				var specificMatches = statement.match(pathPattern);
				var relativePathOfReference = null;
				// Get first value from capturing parentheses
				for (var i = 1; i < specificMatches.length; i ++) {
					if (specificMatches[i] !== null && specificMatches[i] !== undefined) {
						relativePathOfReference = specificMatches[i];
						break;
					}
				}
