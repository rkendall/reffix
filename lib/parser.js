...
var parser = {
	...
	getBrokenReferences: function() {
		var self = this;
		var renamedFilesPromise = gitParser.getHistory();
	...
	getCorrectedPathsForMovedFiles: function() {
		...
		self.brokenReferences.forEach(function(brokenReference) {
			var correctedPath = self.getCorrectedPathFromGit(brokenReference.referencedFile);
			if (correctedPath) {
				brokenReference.correctPath = correctedPath;
				// TODO Can this property be eliminated?
	...
	getCorrectedPathFromGit: function(brokenReference) {
		...
		return this.renamedFiles[brokenReference] || null;
