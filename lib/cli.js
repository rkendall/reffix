...
var cli = {
	...
	parseCommandLine: function() {
		...
		program
			...
			.option('-S, --sources <file_globs>', 'Filter the sources that are analyzed and repaired')
			.option('-T, --targets <file_globs>', 'Filter the targets that are analyzed and repaired')
		...
		var options = {};
		options.workingDir = this.getWorkingDirArg();
	...
	getWorkingDirArg: function() {
		var lastArgNotCapturedByCommander = program.args[program.args.length - 1];
		var lastArg = program.rawArgs[program.rawArgs.length - 1];
		if (lastArgNotCapturedByCommander === lastArg) {
			return lastArg;
		} else {
			return null;
		}
